...
def cosine_similarity(item_1, item_2):
    ...
    return (np.sum(item_1 * item_2, dtype=np.float32)) / (
            np.sqrt(np.sum(item_1 * item_1, dtype=np.float32)) * np.sqrt(
        np.sum(item_2 * item_2, dtype=np.float32)) + 1e-10)


def normalization(vector):
    return vector / (np.sqrt(np.sum(vector * vector, dtype=np.float32)) + 1e-10)
...
class MultiUserEnv(gym.Env):
    ...
    def __init_user__(self):
        # init user interest vector
        normal_user_num = int(self.user_num * 0.1)
        wandering_user_num = self.user_num - normal_user_num
        self.user_interest_vectors = np.concatenate([np.random.uniform(0.0, 1, size=[normal_user_num, self.topic_num]),
                                                     np.zeros(shape=[wandering_user_num, self.topic_num],
                                                              dtype=np.float32)], axis=0)
        print(self.user_interest_vectors.shape)

        # generate user visiting frequency
        channel_visiting_frequency = np.array([
            [0.5, 0.5, 0, 0], [0.5, 0, 0.5, 0], [0.5, 0, 0, 0.5], [0, 0.5, 0, 0.5],
            [0.5, 0.3, 0, 0.2], [0.5, 0, 0.2, 0.3], [0.25, 0.25, 0.25, 0.25]
        ])

        __generate_channel_frequency_idx__ = np.concatenate([np.random.randint(low=0, high=6, size=[normal_user_num]),
                                                             np.ones([wandering_user_num], dtype=np.int32) * 6], axis=0)
        self.user_channel_visiting_frequencies = channel_visiting_frequency[__generate_channel_frequency_idx__, :]
        # generate user request sequences
        self.user_request_matrix = np.zeros(shape=[self.user_num, self.user_max_request_time], dtype=np.int32)
        for uidx in range(normal_user_num):
        ...
        for uidx in range(normal_user_num, self.user_num):
            self.user_request_matrix[uidx] = np.arange(0, self.user_max_request_time) % self.channel_num

        # generate user preference changing rate
        ...
        self.user_dynamic_alphas = np.concatenate(
            [self.user_candidate_alphas[
                 np.argmax(np.random.multinomial(n=1, pvals=self.user_dynamic_p_values, size=normal_user_num),
                           axis=-1)], np.zeros(shape=[wandering_user_num], dtype=np.int32)], axis=0)
        self.user_dynamic_types = len(self.user_candidate_alphas)
        print("user_dynamic_alphas:", self.user_dynamic_alphas.shape)

        # generate user bid level
        ...
        self.all_user_avg_bidprice_levels = np.concatenate(
            [np.random.choice(self.p_user_level, size=[normal_user_num], replace=True),
             np.ones(shape=[wandering_user_num]) * np.min(self.p_user_level, keepdims=False)], axis=0)
