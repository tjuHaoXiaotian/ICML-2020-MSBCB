...
class ROIDQN_interface(LearningBidAgent, PIDAgent):
    def __init__(
            self,
            user_num,
            n_actions,
        ...
        PIDAgent.__init__(self, init_roi=init_roi, default_alpha=1, budget=budget, integration=5)
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('q_eval'):
            ...
            self.roi_sa = self.gmv_sa / ((1 - self.done) * self.cost_sa + self.direct_cost_value + 1e-6)
        ...
        with tf.variable_scope('action'):
            self.roi_a_0 = self.gmv_path_net[:, 0] / (self.cost_path_net[:, 0] + 1e-6)
            self.gmv_a_1 = self.gmv_path_net[:, 1]
            self.roi_thr_times_ecpm_diff = self.gmv_path_net[:, 1] / (self.roi_a_0 + 1e-6) - self.cost_path_net[:, 1]
            positive_roi_thr_times_ecpm_diff = tf.maximum(self.roi_thr_times_ecpm_diff, 0, name="positive_diff")
            self.optimal_bid = positive_roi_thr_times_ecpm_diff / (
                    self.current_step_pctrs * self.probability_of_not_buying + 1e-6) + 0.01

        with tf.variable_scope('roi'):
            roi_min_action_1 = self.gmv_path_net[:, 1] / (self.cost_path_net[:, 1] + self.bid_max_ph + 1e-6)
            roi_action_0 = self.gmv_path_net[:, 0] / (self.cost_path_net[:, 0] + 1e-6)
    ...
    def _build_loss(self):
        ...
        if self.use_prioritized_experience_replay:
            ...
            self.montecarlo_loss = tf.reduce_mean(self.important_sampling_weight_ph *
                                                  tf.squared_difference(self.return_value, self.roi_sa,
            ...
            self.montecarlo_loss = tf.reduce_mean(tf.squared_difference(self.return_value, self.roi_sa,
                                                                        name='MonteCarlo_error'))

        self.gmv_error = tf.abs(self.gmv_path_value - self.gmv_sa)
        self.cost_error = tf.abs(self.restcost_value - self.cost_sa)
        self.montecarlo_error = tf.abs(self.return_value - self.roi_sa)
    ...
    def experience(self, new_trajectory, other_info=None):
        ...
        longterm_gmv, cost_restpath = 0, 0
        ...
        for user_tuple in new_gmv_cost_trajectory[::-1]:
            ...
            longterm_gmv += gmv
            roi_return_value = longterm_gmv / (cost_restpath + cost + 1e-6)
            buffer.add(state, action, longterm_gmv, cost, cost_restpath, roi_return_value, float(done))
            cost_restpath += cost
    ...
    def train_prioritized(self, sess):
        ...
        loss, montecarlo_loss, q_eval, roi_return_values = 0, 0, 0, 0
        ...
        for idx in range(self.update_times_per_train):
            ...
            obses_t, actions, gmv_returns, direct_costs, rest_costs, roi_return_values, dones, weights, ranges = self.prioritized_replay_buffer.sample_index(
                sample_indices)
            ...
            priority_values = sess.run(
                [self._train_op, self.loss, self.montecarlo_loss, self.roi_sa,
                ...
                feed_dict={
                    ...
                    self.return_value: roi_return_values,
                    self.done: dones,
        ...
        return loss, montecarlo_loss, np.average(q_eval), np.average(roi_return_values), \
    ...
    def train_normal(self, sess):
        ...
        for idx in range(self.update_times_per_train):
            ...
            obses_t, actions, gmv_returns, direct_costs, rest_costs, roi_return_values, dones = self.replay_buffer.sample_index(
                sample_indices)
            _, loss, montecarlo_loss, q_eval, \
            _1, gmv_loss, gmv_montecarlo_loss, gmv_q_eval, \
            _2, cost_loss, cost_montecarlo_loss, cost_q_eval \
